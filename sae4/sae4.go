		...
		"Network": &params.Sheet{
			{Sel: "Layer", Desc: "all defaults",
				Params: params.Params{
					...
					"Layer.Act.Noise.Var":                "0.002",     // 0.01 > 0.005 > 0.02
			...
			{Sel: "#Encoding2", Desc: "Encoding uses output's original settings as a base",
				Params: params.Params{
					"Layer.Inhib.Layer.On":      "true", 
					"Layer.Inhib.Layer.Gi":      "1.0",   //									##### KEY PARAMETER ##### 
					"Layer.Inhib.Layer.MaxVsAvg": "0.5", // 									##### KEY PARAMETER #####
					"Layer.Inhib.Pool.On":       "false",  //									##### KEY PARAMETER #####
					"Layer.Inhib.Pool.Gi":       "0.6",  //										##### KEY PARAMETER #####
					"Layer.Act.Noise.Dist":      "Gaussian",
					"Layer.Act.Noise.Var":       "0.00",     // 0.01 > 0.005 > 0.02
			...
			{Sel: "#Encoding", Desc: "Encoding uses output's original settings as a base",
				Params: params.Params{
					"Layer.Act.Noise.Dist":      "Gaussian",
					"Layer.Act.Noise.Var":       "0.005",     // 0.01 > 0.005 > 0.02			##### KEY PARAMETER #####
					"Layer.Act.Noise.Type":      "GeNoise", 

				}},
			{Sel: "#Digit", Desc: "Better not to have LI in the digit layer so classification error can propagate",
				Params: params.Params{
					"Layer.Inhib.Layer.On":      "true",
					"Layer.Inhib.Layer.Gi":      "0.0", 
					"Layer.Inhib.Pool.On":       "false",  
					"Layer.Inhib.Pool.Gi":       "0.0",  
				}},
			...
			{Sel: "#Hidden1ToEncoding", Desc: "Synaptic failure settings",
				Params: params.Params{
					"Prjn.Com.PFail":		"0.0",   // 										##### KEY PARAMETER #####
			}},
			{Sel: "#Encoding2ToDigit", Desc: "",
				Params: params.Params{
					"Prjn.Learn.Learn": 	"false", //
                    "Prjn.SWt.Init.Mean":    "0.5",   // .5 lvis
                    "Prjn.SWt.Init.Var":     "0.00", // seems better
				}},
			{Sel: "#DigitToEncoding2", Desc: "",
				Params: params.Params{
					"Prjn.Learn.Learn":		 "false", //
                    "Prjn.SWt.Init.Mean":    "0.5",   // .5 lvis
                    "Prjn.SWt.Init.Var":     "0.00", // seems better
					"Prjn.PrjnScale.Rel": 	 "0.3", // 0.3 > 0.2 > 0.1 > 0.5 					 ##### KEY PARAMETER #####
...
type Sim struct {
	...
	SpikeLog    *etable.Table `view:"no-inline" desc:"summary log of each run"`
	...
	MCycles int            `desc:"maximum number of epochs to run per model run"`
	...
	SpikeFile    *os.File                    `view:"-" desc:"log file"`
...
func (ss *Sim) New() {
	...
	ss.Time.Defaults()
	ss.ActRFNms = []string{"Encoding2:Output"}
...
func (ss *Sim) ConfigNet(net *axon.Network) {
	net.InitName(net, "AE")
	enc := net.AddLayer4D("Encoding", 1, 10, 5, 1, emer.Hidden)	
	enc2 := net.AddLayer4D("Encoding2", 1, 10, 5, 1, emer.Hidden)	
	digit := net.AddLayer4D("Digit", 1, 10, 1, 1, emer.Target)
	...
	// default is Above, YAlign = Front, XAlign = Center
	 hid1.SetRelPos(relpos.Rel{Rel: relpos.Above, Other:"Encoding", XOffset: -7.5, Space: -5})
	 inp.SetRelPos(relpos.Rel{Rel: relpos.Above, Other:"Hidden1",  XOffset: -16, Space: -5})
 	 enc2.SetRelPos(relpos.Rel{Rel: relpos.RightOf, Other:"Encoding", Space: 2})
	 digit.SetRelPos(relpos.Rel{Rel: relpos.Behind, Other:"Encoding2", Space: 2})
 	 hid2.SetRelPos(relpos.Rel{Rel: relpos.RightOf, Other:"Hidden1", Space: 2})
	 out.SetRelPos(relpos.Rel{Rel: relpos.RightOf, Other:"Input", Space: 2})
	 
//	 hid1.SetRelPos(relpos.Rel{Rel: relpos.Above, Other:"Input", Space: 5})
	...
	net.ConnectLayers(enc, enc2, full, emer.Forward)
//	net.ConnectLayers(enc2, digit, prjn.NewPoolOneToOne(), emer.Forward)

//	net.BidirConnectLayers(hid1, enc, full)//, emer.Forward)
//	net.BidirConnectLayers(enc, hid2, full)//, emer.Forward)
//	net.BidirConnectLayers(hid2, out, full)//, emer.Forward)
//	net.BidirConnectLayers(enc, enc2, prjn.NewPoolOneToOne())//, emer.Forward)
	net.BidirConnectLayers(enc2, digit, prjn.NewPoolOneToOne())//, emer.Forward)
...
func (ss *Sim) ApplyInputs(en env.Env) {
	...
	dig := ss.Net.LayerByName("Digit").(axon.AxonLayer).AsAxon()
	digpats := en.State("Digit")
	dig.ApplyExt(digpats)
...
func (ss *Sim) ConfigPats() {
	...
	dt.SaveCSV("mnist_custom.tsv", etable.Tab, etable.Headers)
}
...
func (ss *Sim) OpenPats() {
	...
	dt.SetMetaData("name", "TrainPats")
	dt.SetMetaData("desc", "Training patterns")
	err := dt.OpenCSV("mnist_train1000.tsv", etable.Tab)
	if err != nil {
		log.Println(err)
	}
}
...
func (ss *Sim) ConfigTrnEpcPlot(plt *eplot.Plot2D, dt *etable.Table) *eplot.Plot2D {
	plt.Params.Title = "Axon Autoencoder Epoch Plot"
...
func (ss *Sim) ConfigTstTrlPlot(plt *eplot.Plot2D, dt *etable.Table) *eplot.Plot2D {
	plt.Params.Title = "Axon Autoencoder Test Trial Plot"
...
func (ss *Sim) ConfigTstEpcPlot(plt *eplot.Plot2D, dt *etable.Table) *eplot.Plot2D {
	plt.Params.Title = "Axon Autoencoder Testing Epoch Plot"
	plt.Params.XAxisCol = "Epoch"
	plt.SetTable(dt)
	// order of params: on, fixMin, min, fixMax, max
	plt.SetColParams("Run", eplot.Off, eplot.FixMin, 0, eplot.FloatMax, 0)
...
func (ss *Sim) ConfigTstCycPlot(plt *eplot.Plot2D, dt *etable.Table) *eplot.Plot2D {
	plt.Params.Title = "Axon Autoencoder Test Cycle Plot"
...
func (ss *Sim) ExportSpikes(dt *etable.Table){
	ly := ss.Net.LayerByName("Encoding")
	lvt := ss.ValsTsr("Encoding")
	ly.UnitValsTensor(lvt, "Spike")
	ly2 := ss.Net.LayerByName("Digit")
	lvt2 := ss.ValsTsr("Digit")
	ly2.UnitValsTensor(lvt2, "Spike")
	row := dt.Rows
	dt.SetNumRows(row + 1)
	dt.SetCellTensor("Spike", row, lvt)
	dt.SetCellTensor("Spike_digit", row, lvt2)
}
...
func (ss *Sim) ConfigSpikeLog(dt *etable.Table) {
	dt.SetMetaData("name", "SpikeLog")
	dt.SetMetaData("desc", "Spike series from model")
	dt.SetMetaData("read-only", "true")
	dt.SetMetaData("precision", strconv.Itoa(LogPrec))
	sch := etable.Schema{
		{"Spike", etensor.FLOAT64, []int{50}, nil},  //set manually until I figure out how to get the tensor length
		{"Spike_digit", etensor.FLOAT64, []int{10}, nil},  //set manually until I figure out how to get the tensor length
...
func (ss *Sim) ConfigRunPlot(plt *eplot.Plot2D, dt *etable.Table) *eplot.Plot2D {
	plt.Params.Title = "Axon Autoencoder Run Plot"
...
func (ss *Sim) ConfigGui() *gi.Window {
	...
	win := gi.NewMainWindow("AE", "Axon Autoencoder", width, height)
	ss.Win = win
...
//	}
	...
	for _, anm := range ss.ActRFNms {
		...
		tnm := sp[1]
		tvt := ss.ValsTsr(tnm)
		ss.ActRFs.Add(anm, lvt, tvt, 0.35) // thr prevent weird artifacts (.01)
	}
}
